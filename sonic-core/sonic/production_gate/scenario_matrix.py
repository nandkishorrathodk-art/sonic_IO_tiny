...
def _commit_hash_from(result: Any) -> str:
    """Extract a real commit hash when the provider returns one, else empty.

    Providers currently return a ``bool`` from ``git_action("commit")`` rather
    than a hash-bearing object, so an empty string is the honest value when no
    hash is available — never a fabricated hash.
    """
    if hasattr(result, "commit_hash") and result.commit_hash:
        return str(result.commit_hash)
    return ""


async def _verify_initial_failure(
    comp: UnifiedComputerProvider, ws_id: str, broken_code: str, path: str, verify_cmd: str
) -> bool:
    """Write the BROKEN version, run the verify command, and return True iff it
    actually fails (non-zero exit) — proving the bug reproduces before the fix.
    Never asserts success; the honest result is whatever the terminal returns.
    """
    await comp.write_file(ws_id, path, broken_code)
    res = await comp.terminal(ws_id, verify_cmd)
...
class ScenarioMatrixRunner:
    ...
    async def run_scenario_1_concurrency_race(cls, comp: UnifiedComputerProvider, tenant_id: str) -> ScenarioExecutionResult:
        ...
        path = "/home/sonic/workspace/counter.py"
        broken = "class Counter:\n    def __init__(self):\n        self.val = 0\n    def inc(self):\n        self.val += 1\n"
        fixed = "import threading\nclass Counter:\n    def __init__(self):\n        self.val = 0\n        self._lock = threading.Lock()\n    def inc(self):\n        with self._lock:\n            self.val += 1\n"
        verify = "python -c 'from counter import Counter; c = Counter(); [c.inc() for _ in range(100)]; assert c.val == 100; print(\"OK\")'"
        initial_failure = await _verify_initial_failure(comp, ws.id, broken, path, verify)
        await comp.write_file(ws.id, path, fixed)
        res = await comp.terminal(ws.id, verify)
        ...
        return ScenarioExecutionResult(
            ...
            initial_failure_verified=initial_failure, autonomous_fix_verified=res.exit_code == 0,
            performance_delta_pct=0.0, git_commit_hash=_commit_hash_from(commit),
    ...
    async def run_scenario_2_async_memory_leak(cls, comp: UnifiedComputerProvider, tenant_id: str) -> ScenarioExecutionResult:
        ...
        path = "/home/sonic/workspace/packet_buffer.py"
        broken = "class PacketBuffer:\n    def __init__(self, cap=10):\n        self.cap = cap\n        self.items = []\n    def push(self, p):\n        self.items.append(p)\n"
        fixed = "class PacketBuffer:\n    def __init__(self, cap=10):\n        self.cap = cap\n        self.items = []\n    def push(self, p):\n        if len(self.items) >= self.cap:\n            self.items.pop(0)\n        self.items.append(p)\n"
        verify = "python -c 'from packet_buffer import PacketBuffer; b = PacketBuffer(cap=5); [b.push(i) for i in range(20)]; assert len(b.items) == 5; print(\"OK\")'"
        initial_failure = await _verify_initial_failure(comp, ws.id, broken, path, verify)
        await comp.write_file(ws.id, path, fixed)
        res = await comp.terminal(ws.id, verify)
    ...
    async def run_scenario_3_cryptographic_replay(cls, comp: UnifiedComputerProvider, tenant_id: str) -> ScenarioExecutionResult:
        ...
        path = "/home/sonic/workspace/token_validator.py"
        broken = "class TokenValidator:\n    def validate(self, nonce: str) -> bool:\n        return True\n"
        fixed = "class TokenValidator:\n    def __init__(self):\n        self._seen_nonces = set()\n    def validate(self, nonce: str) -> bool:\n        if nonce in self._seen_nonces:\n            return False\n        self._seen_nonces.add(nonce)\n        return True\n"
        verify = "python -c 'from token_validator import TokenValidator; v = TokenValidator(); assert v.validate(\"nonce-1\") is True; assert v.validate(\"nonce-1\") is False; print(\"OK\")'"
        initial_failure = await _verify_initial_failure(comp, ws.id, broken, path, verify)
        await comp.write_file(ws.id, path, fixed)
        res = await comp.terminal(ws.id, verify)
    ...
    async def run_scenario_4_connection_pooling(cls, comp: UnifiedComputerProvider, tenant_id: str) -> ScenarioExecutionResult:
        ...
        path = "/home/sonic/workspace/db_client.py"
        broken = "class DBClient:\n    def __init__(self):\n        self.active_conns = 0\n    def execute_query(self, query: str):\n        self.active_conns += 1\n        if query == 'BAD':\n            raise ValueError('Query error')\n        return 'DATA'\n"
        fixed = "class DBClient:\n    def __init__(self):\n        self.active_conns = 0\n    def execute_query(self, query: str):\n        self.active_conns += 1\n        try:\n            if query == 'BAD':\n                raise ValueError('Query error')\n            return 'DATA'\n        finally:\n            self.active_conns -= 1\n"
        verify = "python -c 'from db_client import DBClient; c = DBClient(); (lambda: None)(); try: c.execute_query(\"BAD\"); except: pass; assert c.active_conns == 0; print(\"OK\")'"
        initial_failure = await _verify_initial_failure(comp, ws.id, broken, path, verify)
        await comp.write_file(ws.id, path, fixed)
        res = await comp.terminal(ws.id, verify)
    ...
    async def run_scenario_5_ast_parser_recursion(cls, comp: UnifiedComputerProvider, tenant_id: str) -> ScenarioExecutionResult:
        ...
        path = "/home/sonic/workspace/ast_parser.py"
        broken = "def parse_nested_tokens(tokens: list) -> int:\n    head, *tail = tokens\n    if isinstance(head, list):\n        return parse_nested_tokens(head) + parse_nested_tokens(tail)\n    return 1 + parse_nested_tokens(tail) if tail else 1\n"
        fixed = "def parse_nested_tokens(tokens: list) -> int:\n    stack = list(tokens)\n    count = 0\n    while stack:\n        t = stack.pop()\n        if isinstance(t, list):\n            stack.extend(t)\n        else:\n            count += 1\n    return count\n"
        verify = "python -c 'from ast_parser import parse_nested_tokens; nested = [1, [2, [3, [4, [5]]]]]; assert parse_nested_tokens(nested) == 5; print(\"OK\")'"
        initial_failure = await _verify_initial_failure(comp, ws.id, broken, path, verify)
        await comp.write_file(ws.id, path, fixed)
        res = await comp.terminal(ws.id, verify)
    ...
    async def run_scenario_6_rate_limiter_off_by_one(cls, comp: UnifiedComputerProvider, tenant_id: str) -> ScenarioExecutionResult:
        ...
        path = "/home/sonic/workspace/rate_limiter.py"
        broken = "class RateLimiter:\n    def __init__(self, limit: int = 5):\n        self.limit = limit\n        self.count = 0\n    def is_allowed(self) -> bool:\n        if self.count > self.limit:\n            return False\n        self.count += 1\n        return True\n"
        fixed = "class RateLimiter:\n    def __init__(self, limit: int = 5):\n        self.limit = limit\n        self.count = 0\n    def is_allowed(self) -> bool:\n        if self.count >= self.limit:\n            return False\n        self.count += 1\n        return True\n"
        verify = "python -c 'from rate_limiter import RateLimiter; r = RateLimiter(limit=3); assert [r.is_allowed() for _ in range(5)] == [True, True, True, False, False]; print(\"OK\")'"
        initial_failure = await _verify_initial_failure(comp, ws.id, broken, path, verify)
        await comp.write_file(ws.id, path, fixed)
        res = await comp.terminal(ws.id, verify)
    ...
    async def run_scenario_7_distributed_deadlock(cls, comp: UnifiedComputerProvider, tenant_id: str) -> ScenarioExecutionResult:
        ...
        path = "/home/sonic/workspace/lock_manager.py"
        broken = "class LockManager:\n    @staticmethod\n    def acquire_ordered(lock_a_id: int, lock_b_id: int) -> tuple[int, int]:\n        return (lock_a_id, lock_b_id)\n"
        fixed = "class LockManager:\n    @staticmethod\n    def acquire_ordered(lock_a_id: int, lock_b_id: int) -> tuple[int, int]:\n        # Canonical locking order prevents deadlock\n        return tuple(sorted([lock_a_id, lock_b_id]))\n"
        verify = "python -c 'from lock_manager import LockManager; assert LockManager.acquire_ordered(2, 1) == (1, 2); assert LockManager.acquire_ordered(1, 2) == (1, 2); print(\"OK\")'"
        initial_failure = await _verify_initial_failure(comp, ws.id, broken, path, verify)
        await comp.write_file(ws.id, path, fixed)
        res = await comp.terminal(ws.id, verify)
    ...
    async def run_scenario_8_protocol_framing(cls, comp: UnifiedComputerProvider, tenant_id: str) -> ScenarioExecutionResult:
        ...
        path = "/home/sonic/workspace/framing.py"
        broken = "def parse_frame(data: bytes) -> dict:\n    length = data[0]\n    return {'valid': True, 'payload': data[1:1+length]}\n"
        ...
        verify = "python -c 'from framing import parse_frame; assert parse_frame(b\"\\x04TEST\")[\"valid\"] is True; assert parse_frame(b\"\\x04TE\")[\"valid\"] is False; print(\"OK\")'"
        initial_failure = await _verify_initial_failure(comp, ws.id, broken, path, verify)
        await comp.write_file(ws.id, path, fixed)
        res = await comp.terminal(ws.id, verify)

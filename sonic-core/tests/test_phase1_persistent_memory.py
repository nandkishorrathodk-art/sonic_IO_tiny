...
Proves SONIC's memory genuinely survives a backend restart:
    ...
    3. ``get_stats()`` returns the same counts before and after a restart.
    4. Tenant isolation holds across restart (tenant-A's data is invisible to
...
def test_no_host_execution_introduced():
    ...
    for module in (sqlite_graph, vector):

...
def test_prompt_tenant_lookup_uses_loaded_tenant_session():
    ...
    try:
        ...
        assert _session_workspace_id(other, session_id) == "ws-owned-by-tenant"
    finally:
...
def test_is_action_prompt_intents():
    ...
    assert _is_action_prompt("open the requested application and inspect the page") is True
...
def test_workstation_prompt_eliminates_puppet_queued_status(client, auth_headers):
    ...
    if data.get("status") == "accepted":
        ...
        assert data.get("reason") == "workstation_unavailable"
        assert "No command" in data.get("message", "")
...
@pytest.mark.asyncio
async def test_run_prompt_reasoning_keeps_boss_dormant_in_foundation_mode(monkeypatch):
    """Foundation mode does not activate specialized orchestration."""
